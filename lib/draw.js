...
const drawGrid = () => {
    ...
    const adjustedHeight = 2*two.height - ((2*two.height)%game.gridSize);
    const adjustedWidth = 2*two.width - ((2*two.width)%game.gridSize);
    for (let i = -adjustedWidth*.5; i < adjustedWidth; i += game.gridSize) {
        const line = two.makeLine(i, -adjustedHeight*.5, i, adjustedHeight);
        line.stroke = "#333";
        grid.add(line);
    }
    for (let i = -adjustedHeight*.5; i < adjustedHeight; i += game.gridSize) {
